...
- (void)sendWeiboWithText:(NSString *)text
{
    ...
    AFHTTPRequestOperation *opertion = [DataService requestWithURL:urlStr
                         params:params
                     httpMethod:@"POST"
...
//                       [self completeHUD:@"发送失败"];
    ...
    //点击提交发送微博后，就返回到主界面，提高用户体验，然后在视图的上面添加一个进度条和提示框把上传信息显示给用户
    [self closeAction];
    
    //拿到代理
    AppDelegate *delegate = (AppDelegate *)[UIApplication sharedApplication].delegate;
    //创建UIWindow必要要有一个类持有它的所有权
    delegate.secondWindow = [[UIWindow alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 20)];
    //必须要设置UIWindow的隐藏值为NO，默认为YES
    ...
    //使用第三方"UIProgressView+AFNetworking.h文件，把网络请求的返回值opertion给progress，就可以返回上传的进度
