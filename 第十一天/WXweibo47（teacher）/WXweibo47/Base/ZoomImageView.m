...
//    _fullImageView.frame = rect;
    ...
    [UIView animateWithDuration:0.35 animations:^{
        ...
        //代理协议
        if ([self.zoomDelegate respondsToSelector:@selector(zoomImageViewDidIn)]) {
            //执行代理方法
            [self.zoomDelegate zoomImageViewDidIn];
        };
...
- (void)zoomOutImageView:(UITapGestureRecognizer *)tap
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        //安全判断
        if ([self.zoomDelegate respondsToSelector:@selector(zoomImageViewDidOut)]) {
            //缩小调用的代理方法
            [self.zoomDelegate zoomImageViewDidOut];
        }
