...
- (void)buttonAction:(ThemeButton *)btn
{
    if (btn.tag == 10) {
        ...
        //这里会造成循环引用，所以要使用中间变量来替换self
        //self -> _faceView -> block -> _textView
        __weak SendViewController *weakThis = self;
        
        if (_faceScrollView == nil) {
            _faceScrollView = [[FaceScrollView alloc] initWithBlock:^(NSString *faceName) {
                //...
                __strong SendViewController *strongThis = weakThis;
                //这里一定要做安全判断
                if (faceName != nil) {
                    //直接插入到当前的文本框中(新方法)
                    [strongThis->_textView insertText:faceName];
                }
        ...
        if (!btn.selected) {
            //表情视图作为键盘的一部分
            _textView.inputView = _faceScrollView;
